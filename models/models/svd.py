...
def retrain_model():
    ...
    metrics = pd.DataFrame(columns=["date", "rmse", "mae", "rsquared", "xp_var", "MAP", "NDCG", "Precision@K", "Recall@k"])
    list = [datetime.datetime.now(),eval_rmse, eval_mae, eval_rsquared, eval_exp_var,
                 eval_map, eval_ndcg, eval_precision, eval_recall]
    metrics.loc[len(metrics)] = list
    metrics.to_sql("metrics", con=engine, if_exists='append', index=False)
retrain_model()